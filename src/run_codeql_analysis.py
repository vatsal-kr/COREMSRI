...
def get_unfixed_files(query_metadata, files_subset, args):
    ...
    for query in Queries:
        ...
        unfixed_in_set = 0
        try:
            csv_result_file = args.codeql_result_path + "/" + f"{query_folderName}/results_{query_folderName}.csv"
        ...
        list_of_files = copy.deepcopy(files_subset[query][0] + files_subset[query][1] + files_subset[query][2] + files_subset[query][3] + files_subset[query][4])
        ...
        for file_unfixed in set_unique_files_names:
            ...
            if file_check in list_of_files:
                ...
                top_effective = get_file_fixes(query, file_check, query_metadata, args.generated_folder_path)
...
def get_files_with_syntax_errors(query, files_subset, legends, query_metadata, args):
    top = args.n
    ...
    log_dir = args.codeql_db_path + "/" + f"{query_metadata[query]['folder_name']}/python-database/log/"
    ...
    for file_with_error in files_with_syntax_error:
        if not os.path.exists(file_with_error):
            logging.critical("danger: ", file_with_error)
            sys.exit(-1)
    ...
    # save the splits
    files_subset_with_syntax_error[query] = {}  # all unfixed <editede> files in 4 categories
...
def get_considered_files(Queries, query_metadata, root_folder, generated_files_folder):
    considered_files = {}
    ...
    for query in Queries:
        ...
        generated_py_files_in_query = set([(x.split("_")[0] + ".py") for x in generated_files_in_query if x.endswith(".py")])

        considered_files[query] = py_files_in_query.intersection(generated_py_files_in_query)
        files_with_no_generated_files[query] = py_files_in_query - generated_py_files_in_query
...
def get_file_fixes(query, org_file, query_metadata, generated_files_folder):
    ...
    generated_files_in_query = set(os.listdir(gff))
    file_generated_py_files_in_query = set([x for x in generated_files_in_query if ((x.endswith(".py")) and (org_file == x.split("_")[0] + ".py"))])
...
def get_plot_data(query_metadata, files_subset, query_unfixed_files, args):
    ...
    aggregate_data = {}
    considered_files, files_with_no_generated_files = get_considered_files(Queries, query_metadata, args.folder_path, args.generated_folder_path)
    ...
    for query in Queries:
        ...
        edited_files_with_syntax_error, files_subset_with_syntax_error = get_files_with_syntax_errors(query, files_subset, legends, query_metadata, args)
        # get stats
        valid = 0
        not_valid = 0
        total = 0
        ...
        for i in range(len(files_subset[query])):
            query_bucket_specific_considered_files = set(files_subset[query][i]).intersection(considered_files[query])
            # get numbers of syntacticallly corret or incorrect files

            temp_valid = 0
            temp_not_valid = 0
            for file_name in query_bucket_specific_considered_files:
                top_effective = get_file_fixes(query, file_name, query_metadata, args.generated_folder_path)
                if file_name in edited_files_with_syntax_error and len(edited_files_with_syntax_error[file_name]) == min(top, top_effective):
                    temp_not_valid += 1
            ...
            if i in [0, 1, 2, 3, 4]:
                ...
                files_wo_syntax_error = query_bucket_specific_considered_files - set(
                    [x for x in files_subset_with_syntax_error[query][legends[i]].keys() if len(files_subset_with_syntax_error[query][legends[i]][x]) == min(top, get_file_fixes(query, x, query_metadata, args.generated_folder_path))]
                )
                ...
                for ff in files_wo_syntax_error:
                    top_effective = get_file_fixes(query, ff, query_metadata, args.generated_folder_path)
                    # files in files_wo_syntax_error may have some syn. incorrect files
                    if ff in edited_files_with_syntax_error and ff in query_unfixed_files[query][3]:
                        if len(edited_files_with_syntax_error[ff]) + len(query_unfixed_files[query][3][ff]) == min(top, top_effective):
                            unfixed_ones += 1
                    ...
                    # files in files_wo_syntax_error with 0 syn. incorrect files, but no fixed files
                        ...
                        if len(query_unfixed_files[query][3][ff]) == min(top, top_effective):
                            unfixed_ones += 1
                            unfixed_files.append(ff)
                        else:
                            fixed_ones += 1
                ...
                for fg in query_valid_files:
                    if fg[1] == i and fg not in query_valid_unfixed_files:
                        ...
                        if fg[0] in edited_files_with_syntax_error:
                            _plausible_all_files = set(x for x in [fg[0].split(".py")[0] + f"_{i}.py" for i in range(top)] if x not in edited_files_with_syntax_error[fg[0]])
                        else:
                            _plausible_all_files = set([fg[0].split(".py")[0] + f"_{i}.py" for i in range(top)])
                        ...
                        if fg[0] in unfixed_files_dict:
                            if _all_files - set(unfixed_files_dict[fg[0]]):
                                query_valid_fixed_files[fg] = _all_files - set(unfixed_files_dict[fg[0]])
                        else:
                ...
                aggregate_data = update_aggregate_values(aggregate_data, legends[i], "syn. correct", "None", temp_valid)

            query_stats[query]["types"].append(legends[i])
            query_stats[query]["validity"].append("syn. incorrect")
            query_stats[query]["fixed"].append(None)
            query_stats[query]["values"].append(temp_not_valid)
            aggregate_data = update_aggregate_values(aggregate_data, legends[i], "syn. incorrect", "None", temp_not_valid)
        ...
        edited_file_lists[query]["total"] = len(considered_files[query]) + len(  # sum of invalid, valid_fixed, valid_unfixed
...
def generate_pickle_file(args):
    ...
    query_stats, _, edited_file_lists = get_plot_data(query_metadata, files_subset, query_unfixed_files, args)
    ...
    for query in query_stats.keys():
        ...
        if query != "Aggregate":
            fixed = sum(df[(df["valid"] == "syn. correct") & (df["fixed"] == "fixed")]["value"])
            total = sum(df["value"])
    ...
    pkl_file_path = Path(args.output_path) / Path(args.description + "_filefix_types_Top" + str(args.n) + ".pickle")
